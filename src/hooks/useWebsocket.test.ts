import { getBuiltTeam, getSwappedPkm } from "./websocket.functions";
const randomTest = {
...
test("non random battle selector works", () => {
  const teams = getBuiltTeam(ouTest.test);
  expect(teams).toBeTruthy();
  if (teams) {
    expect(teams["p1"]).toStrictEqual(ouTest.results["p1"]);
    expect(teams["p2"]).toStrictEqual(ouTest.results["p2"]);
  }
});
test("random battle selector works", () => {
  const teams = getSwappedPkm(randomTest.test);
  expect(teams).toBeTruthy();
  console.log(teams);
  if (teams) {
    // expect(teams["p1"]).toStrictEqual(ouTest.results["p1"]);
